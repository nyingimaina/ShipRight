...
public class MariaDbCredentialResourceStore : ICredentialResourceStore
{
    ...
    public MariaDbCredentialResourceStore(IDatabaseHelper<Guid> db)
    {
        _db = db;
    }
    ...
    private static CredentialResource Deserialize(Record record)
    {
        return JsonSerializer.Deserialize<CredentialResource>(record.Data, JsonOpts)
            ?? throw new InvalidOperationException($"Failed to deserialize credential resource {record.Id}");
    }
