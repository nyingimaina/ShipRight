...
public class MariaDbPipelineResourceStore : IPipelineResourceStore
{
    ...
    public MariaDbPipelineResourceStore(IDatabaseHelper<Guid> db)
    {
        _db = db;
    }
    ...
    public async Task<List<PipelineResource>> GetAllAsync()
    {
        using var qb = AppQBuilderExtensions.NewQBuilder();
        var built = qb
    ...
    public async Task<List<PipelineResource>> GetGlobalAsync()
    {
        using var qb = AppQBuilderExtensions.NewQBuilder();
        var built = qb
            .UseSelector().Select<Record>("*").Then()
    ...
    public async Task<List<PipelineResource>> GetByProjectAsync(string projectId)
    {
        using var qb = AppQBuilderExtensions.NewQBuilder();
        var built = qb
            .UseSelector().Select<Record>("*").Then()
    ...
    public async Task<PipelineResource?> GetByIdAsync(Guid id)
    {
        using var qb = AppQBuilderExtensions.NewQBuilder();
        var built = qb
            .UseSelector().Select<Record>("*").Then()
    ...
    public async Task<PipelineResource?> GetByNameAsync(string name)
    {
        using var qb = AppQBuilderExtensions.NewQBuilder();
        var built = qb
            .UseSelector().Select<Record>("*").Then()
    ...
    private static PipelineResource Deserialize(Record record)
    {
        return JsonSerializer.Deserialize<PipelineResource>(record.Data, JsonOpts)
            ?? throw new InvalidOperationException($"Failed to deserialize pipeline resource {record.Id}");
    }

    [Table("PipelineResource")]
